...
def azure_openai_request(conversation_history):
    url = f"{openai.api_base}/openai/deployments/{openai_api_deployment}/chat/completions?api-version={openai_api_version}"
    headers = {
...
# Traiter l'entrée utilisateur et obtenir une réponse
if user_input:
...
if conversation_history[-1]["role"] == "assistant":
    st.write(f"\n**Elara:** {conversation_history[-1]['content']}")